...
class Generator(object):
    ...
    def __init__(self, discrim: discriminator.Discriminator):
        ...
        self.W_conv1 = Util.__weight_variable([5, 5, config.NUM_CHANNELS, 32])
        self.b_conv1 = Util.__bias_variable([32])
        self.h_conv1 = tf.nn.relu(Util.__conv2d(self.x_image, self.W_conv1) + self.b_conv1)  # 128 x 128
        self.h_pool1 = Util.__max_pool_2x2(self.h_conv1)  # 64 x 64

        # convolutions, biases, activations, and pool of 2nd layer
        self.W_conv2 = Util.__weight_variable([5, 5, 32, 64])
        self.b_conv2 = Util.__bias_variable([64])
        self.h_conv2 = tf.nn.relu(Util.__conv2d(self.h_pool1, self.W_conv2) + self.b_conv2)  # 64 x 64
        self.h_pool2 = Util.__max_pool_2x2(self.h_conv2)  # 32 x 32

        # convolutions, biases, activations, and pool of 3rd layer
        self.W_conv3 = Util.__weight_variable([5, 5, 64, 64])
        self.b_conv3 = Util.__bias_variable([64])
        self.h_conv3 = tf.nn.relu(Util.__conv2d(self.h_pool2, self.W_conv3) + self.b_conv3)  # 32 x 32
        self.h_pool3 = Util.__max_pool_2x2(self.h_conv3)  # 16 x 16

        # convolutions, biases, activations, and pool of 4th layer
        self.W_conv4 = Util.__weight_variable([5, 5, 64, 64])
        self.b_conv4 = Util.__bias_variable([64])
        self.h_conv4 = tf.nn.relu(Util.__conv2d(self.h_pool3, self.W_conv4) + self.b_conv4)  # 16 x 16
        self.h_pool4 = Util.__max_pool_2x2(self.h_conv4)  # 8 x 8
        ...
        self.W_fc1 = Util.__weight_variable([8 * 8 * 64, config.IM_SIZE_X * config.IM_SIZE_Y])
        self.b_fc1 = Util.__bias_variable([config.IM_SIZE_X * config.IM_SIZE_Y])
        ...
        self.W_fc2 = Util.__weight_variable([config.IM_SIZE_X * config.IM_SIZE_Y, config.NUM_CLASSES])
        self.b_fc2 = Util.__bias_variable([config.NUM_CLASSES])
